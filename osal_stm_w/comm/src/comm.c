...
static CommTypeDef* pthis[COMM_CHANNEL_SIZE] = {NULL};
...
#endif
	pthis[COMM_CHANNEL0] = &m_Instance;
}
...
{
	if (NULL == pthis[COMM_CHANNEL0])
	{
		New();
	}
	return pthis[eChannel];
}
...
{
	if (pthis[eChannel] != NULL)
		pthis[eChannel]->deInit();
	
	pthis[eChannel] = hCommHandle;
